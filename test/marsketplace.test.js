  ...
  : describe("Unit MarsKetplace tests", function () {
      ...
      describe("List item function", async () => {
        it("Should revert if price is 0", async () => {
          await expect(
            NftMarsketPlace.listNft(oneNftDeployed.address, TOKENID, 0)
        ...
        //   await expect(
        //     NftMarsketPlace.listNft(oneNftDeployed.address, TOKENID, price)
        ...
        it("Should list and emit an event ItemListed", async () => {
          ...
          expect(await NftMarsketPlace.listNft(oneNftDeployed.address, TOKENID, price)).to.emit(
            event
          )
        })
        ...
        it("Should only allow the owner to list an NFT", async () => {
          ...
          await expect(
            buyerConnected.listNft(oneNftDeployed.address, TOKENID, price)
          ).to.be.revertedWith("MarsKetplace__NotOwner")
        })

        it("Should revert if the NFT is already listed", async () => {
          await NftMarsketPlace.listNft(oneNftDeployed.address, TOKENID, price)
